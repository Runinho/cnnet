...
void free_model(model_t** model){
	...
	while(c_layer != NULL && !c_layer->is_last){
...
void train(model_t* model, tensor_handle_t* input, tensor_handle_t* y, int epochs, float learning_rate){
	for(int epoch=0; epoch < epochs; epoch++){
		...
		printf("epoch %d loss: %f", epoch, sum/size);
		...
		while(c_layer != NULL && c_layer != model->first_layer){
...
void print_model(model_t* model){
	...
	while(c_layer != NULL && !c_layer->is_last){
		...
		printf("Layer %d shape: [%d, %d]", layer_index, shape[0], shape[1]);
		// go to next layer
		c_layer = c_layer->next_layer;
		layer_index++;
	}
